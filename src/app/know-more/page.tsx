...
//     // Your code that uses the window object here
...
const KnowMore = () => {
  ...
  const form = useForm<ContactFormValues>({
    ...
    defaultValues: {
      ...
      country: "",
      message: "",
      // privacyAgreement: false, // Assuming you want to handle the checkbox.
    },
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setSearchParams(queryParams); // Set the URLSearchParams instance
    }
  }, []);

  // Access query params once searchParams is set
  const imageUrl = searchParams?.get("imageUrl");
  const heading = searchParams?.get("heading");
  const description = searchParams?.get("description");
  ...
  return (
    <div>
      ...
      <div className="relative h-[70vh] md:h-screen w-full">
        <Image
          src={imageUrl || "/assets/images/self-sustainable-home-hero.jpg"}
          alt="Charming cottage style house with steep roof and surrounded by greenery"
          fill
          priority
          className="object-cover"
